...
interface GeneralInquiriesFilterProps {
  open: boolean;
  onClose: () => void;
  onSave: (cols: ColumnOption[]) => void;
}

/** The default column options, if you want them inside the component,
 * or pass them in as a prop if you'd like.
 */
const defaultColumnOptions: ColumnOption[] = [
  { name: "PO", checked: true },
...
export default function GeneralInquiriesFilter({
  open,
  onClose,
  onSave,
}: GeneralInquiriesFilterProps) {
  // We keep local state for the checkboxes
  const [columns, setColumns] =
    React.useState<ColumnOption[]>(defaultColumnOptions);
