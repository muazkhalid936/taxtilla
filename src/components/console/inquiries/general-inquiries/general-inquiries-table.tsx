...
import { getCustomerColumns, getSupplierColumns, Inquiry } from "./columns";

// Extend the props to include selection state
interface GeneralInquiriesTableProps {
  data: Inquiry[];
  setData: React.Dispatch<React.SetStateAction<Inquiry[]>>;
  role: "customer" | "supplier";
  selectedInquiries?: Inquiry[];
  setSelectedInquiries?: React.Dispatch<React.SetStateAction<Inquiry[]>>;
}

export default function GeneralInquiriesTable({
  data,
  setData,
  role,
  selectedInquiries,
  ...
  const tableColumns = columns.map((col) => {
    if (col.id === "actions") {
      return {
        ...
        cell: ({ row }: any) => {
          const item = row.original as Inquiry;
